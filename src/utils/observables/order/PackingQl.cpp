...
double PackingQl::observe(const Box& box, std::vector<Shell>& shells, const DomainList& dl)
{
    if (box.pbc == false)
    {
        return 0.0;
    ...
    {
        qlsum += PackingQl::calcQl(box, shells, i, dl);
    }
...
double PackingQl::calcQl(const Box& box, std::vector<Shell>& shells, unsigned int ci, const DomainList& dl)
{
