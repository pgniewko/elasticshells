...
* ```run_fem.sh``` - Script to compress 5 elastic shells in a rigid box. Change ```$PBC``` variable for periodic boundary condition.
* ```restart.sh``` - Resart the simulation setup from the ```run_fem.sh``` script.
* ```analyze.sh``` - Read the trajectory generated by ```run_fem.sh``` script, calculate observables defined in ```observers.config``` configuration file and save in an output file.
