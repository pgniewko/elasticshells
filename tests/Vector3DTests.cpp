...
void Vector3DTests::setUp() 
{
    ...
    v5 = new Vector3D(DELTA, DELTA, DELTA);
...
void Vector3DTests::testConstructor()
{
    ...
    Vector3D v3d3 = *v5;
    CPPUNIT_ASSERT_DOUBLES_EQUAL(DELTA, v3d3.x, 0.0);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(DELTA, v3d3.y, 0.0);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(DELTA, v3d3.z, 0.0);
...
void Vector3DTests::testSetLength()
{
    ...
    Vector3D v3d2(*v5);
    v3d2.setLength(1.0);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(1.0, v3d2.length(), DELTA);
    
    Vector3D v3d3(*v2);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(0.0, v3d3.length(), DELTA);
}

void Vector3DTests::testAngle()
{
    CPPUNIT_ASSERT_DOUBLES_EQUAL(0.0, v5->angle(*v5), DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(0.0, v6->angle(*v6), DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(PI/2, v6->angle(*v7), DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(PI/2, v7->angle(*v8), DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(PI, v6->angle(*v8), DELTA);
}
...
void Vector3DTests::testOperators()
{
    ...
    CPPUNIT_ASSERT_DOUBLES_EQUAL(0.0, v3d1.length(), DELTA);
    v3d1 -= *v3;
    CPPUNIT_ASSERT_DOUBLES_EQUAL(-1.0, v3d1.x, DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(1.0, v3d1.y, DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(1.0, v3d1.z, DELTA);
    v3d1 *= -1.0;
    CPPUNIT_ASSERT_DOUBLES_EQUAL(1.0, v3d1.x, DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(-1.0, v3d1.y, DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(-1.0, v3d1.z, DELTA);    
    v3d1 /= 2.0;
    CPPUNIT_ASSERT_DOUBLES_EQUAL(0.5, v3d1.x, DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(-0.5, v3d1.y, DELTA);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(-0.5, v3d1.z, DELTA);  
}